...
export const Menu = ({ setIsOpen }) => {
	...
	const [isMax1000, setIsMax1000] = useState();
	console.log(isMax1000);
	useEffect(() => {
		const updateMediaQuery = () => {
			setIsMax1000(window.matchMedia('(max-width: 1000px)').matches);
		};

		// Ascultă evenimentul de redimensionare a ferestrei pentru a actualiza starea media query
		window.addEventListener('resize', updateMediaQuery);

		// Inițializare starea media query la montarea componentei
		updateMediaQuery();
	...
	return (
		<>
			...
			<div className={cn(styles.menu__hamburger, { [styles.visible__modal]: visibleMenuMobile })}>
			...
			<div className={styles.mobile__search}>
				<div className={styles.top__inner}>
					<InputSearch setVisibleInput={setVisibleInput}></InputSearch>
					<span className={styles.cancel}> Отменить</span>
				</div>
			</div>
		</>
	);
